...
function objMainFrame:new(fParent, tTexture, oSettings, oDisplay)
	...
	obj.SetTomTomDestination = function(self, nX, nY, sZone, title)
		if not ZoneMapIDs then
			return
		end

		TomTom:RemoveAllWaypoints()

		local zoneName = sZone
		local x, y = nX, nY
		...
		local continentId = ZoneMapIDs:GetContinentID(zoneName)
		local zoneIndex = ZoneMapIDs:GetZoneIndex(zoneName)

		if continentId and zoneIndex then
			local playerC, playerZ, playerX, playerY = Astrolabe:GetCurrentPlayerPosition()
			if not (playerC and playerZ and playerX and playerY) then
				return
			end
			TomTom:AddMFWaypoint(continentId, zoneIndex, x / 100, y / 100, { title = title })
		end
	end
	VGuide.SetTomTomDestination = obj.SetTomTomDestination
